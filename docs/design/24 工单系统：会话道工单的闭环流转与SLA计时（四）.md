---
title: "24 工单系统：会话道工单的闭环流转与SLA计时（四）"
source: "https://articles.zsxq.com/id_9587xkztsrmk.html"
author:
  - "[[苏三]]"
published:
created: 2026-09-25
...
### 文件：yunti-frontend/src/views/tickets/index.vue

一个页面里装了：

① **看板数字** + 页签（企业内部工单 / 提交给平台支持）；

② **筛选 + 服务端分页**（状态/优先级/SLA/分类/处理人/关键字，翻页、改每页条数都会回到第 1 页）；

③ **表格 / 看板两种视图**：看板按状态分五列，卡片**拖拽即改状态**；

④ **详情抽屉**：双段 SLA 进度、来源会话一键跳工作台、流转时间线、回复/内部备注、认领/转派/升级/状态流转、平台工单的"补充说明"；

⑤ **角色权限**：只读角色看不到认领/回复/流转，SLA 规则按钮只有管理员能看到；

⑥ **导出 CSV**、**新建平台工单**（标题与按钮都跟着类型变）。
...
### 文件：yunti-frontend/src/views/platform/support/index.vue

平台运营/平台管理员用的页面：跨租户列表（租户号可前缀筛）、看板数字、详情抽屉（提交企业与提交人、双方可见的流转时间线、平台回复、认领、升级、状态流转）、导出 CSV。
...
### 5.4 菜单、路由、工作台入口、消息中心

### 改动：yunti-frontend/src/router/index.ts

改动点：新增工单中心与平台支持工单路由（`/modules/tickets`、`/platform/support`）
...
改动点：工单中心菜单 + 待办/超时角标；平台侧「支持工单」菜单 + 角标；消息中心接后端（新增「工单提醒」页签）

菜单分工：企业账号（`userType=2`）看「运营管理 → 工单中心」，平台账号（`userType=1`）看「支持工单」——两边互相看不到对方的菜单，和后端的鉴权口径一致。
...
``` code-block-container
        ...
        <el-menu-item v-if="userStore.userType === 1" index="/platform/support">
          ...
          <span>支持工单</span>
          <span v-if="platformTodo" class="menu-tip" :class="platformOverdue ? 'danger' : 'warning'">
            {{ platformOverdue ? `超时 ${platformOverdue}` : `待处理 ${platformTodo}` }}
          </span>
        </el-menu-item>
        <template v-if="userStore.userType === 2">
          <el-menu-item v-if="canManageMembers" index="/enterprise">
...
``` code-block-container
          <el-sub-menu index="ops">
            <template #title><span class="group-label">运营管理</span></template>
            <el-menu-item index="/modules/kb"><el-icon><Notebook /></el-icon><span>知识库</span></el-menu-item>
            <el-menu-item index="/modules/tickets">
              <el-icon><Tickets /></el-icon>
              <span>工单中心</span>
              <span v-if="ticketTodo" class="menu-tip" :class="ticketOverdue ? 'danger' : 'warning'">
...
interface NoticeItem {
  id: string
  type: 'system' | 'review' | 'qa' | 'ticket'
  title: string
  text: string
  time: string
  read: boolean
  /** 来自后端消息中心（工单提醒）；本地的是演示数据 */
  source?: 'server' | 'local'
  /** 后端消息的跳转标识：tickets / platform-support */
...
/** 待处理的实时质检预警条数：挂成菜单角标，有没有没处理的预警一眼就看到 */
const alertPending = ref(0)
/** 工单待办：待处理 + 处理中 + 待确认；其中超时单单独标红（客户催之前系统先催） */
const ticketTodo = ref(0)
const ticketOverdue = ref(0)
/** 平台侧支持工单待办：平台账号才拉 */
const platformTodo = ref(0)
const platformOverdue = ref(0)
const canManageMembers = ref(false)
const currentRoleName = ref('')
const notifyTab = ref<'all' | 'ticket' | 'system' | 'review' | 'qa'>('all')

const notifyTabs = [
  { key: 'all' as const, label: '全部' },
  { key: 'ticket' as const, label: '工单提醒' },
  { key: 'system' as const, label: '系统通知' },
  { key: 'review' as const, label: '审核通知' },
  { key: 'qa' as const, label: '质检通知' },
```
 ...
 * 演示用消息（系统 / 审核 / 质检）。
 *
 * <p>工单提醒走真实后端（notification 表），这几条保留是为了让消息中心的其它页签有内容可看。</p>
 */
const demoNotices = reactive<NoticeItem[]>([
  {
    id: 'n1',
    type: 'system',
```
...
/** 后端消息（工单提醒等真实数据） */
const serverNotices = ref<NoticeItem[]>([])
/** 展示用列表：真实消息在前，演示消息在后 */
const notices = computed<NoticeItem[]>(() => [...serverNotices.value, ...demoNotices])

const filteredNotices = computed(() => {
  if (notifyTab.value === 'all') return notices.value
  return notices.value.filter((n) => n.type === notifyTab.value)
})
...
``` code-block-container
    ...
    if (userStore.userType === 1) {
      // 平台账号：只关心支持工单的待办角标
      void loadPlatformTodo()
      platformTimer = window.setInterval(() => void loadPlatformTodo(), 60_000)
      return
    }
    if (userStore.userType === 2) {
      try {
...
``` code-block-container
    ...
    if (!canManageMembers.value) {
      ...
      // 但工单待办和他们直接相关，角标照样要拉
      void loadTicketTodo()
      ticketTimer = window.setInterval(() => void loadTicketTodo(), 60_000)
...
/** 工单待办数：和质检预警一样 60 秒轮询一次（工单页自己也有一套 30 秒的看板刷新） */
async function loadTicketTodo() {
  try {
    const overview = await fetchTicketOverview()
    ticketTodo.value = overview.pending + overview.processing + overview.confirming
    ticketOverdue.value = overview.overdue
  } catch {
    // 拿不到就不显示角标
  }
}

/** 平台侧支持工单待办（平台账号专用，60 秒轮询） */
async function loadPlatformTodo() {
  try {
    ...
    platformOverdue.value = overview.overdue
  } catch {
    // 拿不到就不显示角标
  }
}
...
/**
 * 拉后端消息（工单提醒：SLA 预警 / 分派 / 升级 / 平台回复）。
 *
 * <p>打不开抽屉时也拉一次——右上角红点要有数字，不能等用户点开才知道有新消息。</p>
 */
async function loadNotifications() {
  ...
  try {
    ...
    serverNotices.value = rows.map((row) => ({
      id: row.id,
...
function tabCount(key: string) {
  if (key === 'all') return unreadCount.value
  return notices.value.filter((n) => n.type === key && !n.read).length
}

/** 点开一条消息：本地演示条目走 localStorage，后端消息调接口 */
async function markRead(item: NoticeItem) {
  ...
  if (item.source === 'server') {
    try {
      ...
      // 标记失败不改本地状态：下次打开还会显示未读，不会"假装看过了"
    }
    // 工单类提醒点开就跳到工单中心（消息里说的是哪张单，页面里能直接看）
    if (item.linkView) {
      notifyOpen.value = false
      // platform-support（平台回复/升级了你的支持工单）直接落到「提交给平台支持」页签
      void router.push(item.linkView === 'platform-support'
...
async function markAllRead() {
  const serverIds = serverNotices.value.filter((n) => !n.read).length
  if (serverIds > 0) {
    try {
      await markAllNotificationsRead()
      serverNotices.value.forEach((n) => {
        n.read = true
      })
    } catch {
      // 接口失败就只清本地演示消息，界面不会假装全读
    }
  }
  demoNotices.forEach((n) => {
    n.read = true
  })
  persistReadIds()
}
...
function persistReadIds() {
  localStorage.setItem(NOTIFY_READ_KEY,
    JSON.stringify(demoNotices.filter((n) => n.read).map((n) => n.id)))
}
...
.type-review { background: #e0f2fe; color: #0284c7; }
.type-qa { background: #dbeafe; color: #1d4ed8; }
/* 工单提醒：暖色，跟系统/审核/质检的蓝色系区分开 */
.type-ticket { background: #fff7ed; color: #ea580c; }
...
改动点：会话头部新增「转工单」入口；收到 TICKET\_ALERT 时弹通知并可跳工单中心

会话转工单会带三个参数过去：会话号、客户名、最近一句客户消息（工单页据此预填）；后端还会把来源会话与最近 12 条对话摘要写进工单描述，接单的人不用再问一遍。
...
改动点：开发代理新增 /api/platform（否则本地访问平台接口会返回 index.html）
...
改动点：修掉"图片上传完界面还停在『图片上传中…』"的响应式坑（联调抓出来的真问题）

这个坑值得单独说：Vue 的 `ref([])` 是深响应式的，`push` 进去后数组里存的是**代理**；如果上传完成后改的是外面那个**原始对象**，就绕过了代理的 set，界面不会重渲染——图早就传完了、`fileId` 也拿到了，页面却一直转圈。正确写法是按 `previewUrl` 回查数组里的元素（拿到的是代理）再赋值。
...
``` code-block-container
    ...
    // 上传是异步的：这期间客户可能已经把这张移除了，所以按 previewUrl 回查——
    // 注意回查拿到的是**数组里的响应式对象**，必须改它才会触发重渲染。
    // （直接改上面那个原始 item 是个坑：Vue 存进数组的是它的代理，
    //   改原始对象绕过了代理的 set，界面永远停在"图片上传中…"）
...
``` code-block-container
# 1) 建表/补列：工单这一篇有增量脚本，第一次跑必须执行（幂等，可重复执行）
bash scripts/migrate-customer-db.sh

# 2) 起服务：网关(9090) + customer-service(9093) + user-service(9091) +
#    realtime-service(9096) + yunti-ai(9100) + 前端(5173)
...
**① 会话转工单**：工作台选一条会话 → 头部「转工单」→ 工单页自动弹出「新建工单」并预填（会话号、客户名、最近一句）→ 选优先级"高"、点创建。

  

<img src="https://article-images.zsxq.com/Fv_N48JmKBavyh9PMGTZ1ipUbpfX" class="tiptap-image" alt="图片.png" />

  

<img src="https://article-images.zsxq.com/FqIv17zn-s7fv24WgaCIEWRVsAQv" class="tiptap-image" alt="图片.png" />

  

<img src="https://article-images.zsxq.com/Fm34a9wcKLRjqbyKGYvzxAidj5VB" class="tiptap-image" alt="图片.png" />

  

<img src="https://article-images.zsxq.com/FiYMagq9iMh4mskPkeKJrLGD_vFq" class="tiptap-image" alt="图片.png" />

预期：工单号 TK 开头、来源显示"在线会话"、SLA 显示"正常 · 还剩 30 分钟"，描述里带【来源会话】【客户】【机器人判断】【最近对话】。

**② 闭环动作**：详情抽屉里「认领」→ 状态变处理中；写一条回复（勾着"对客户可见"）→ SLA 的首次响应变成"已响应"；再点「标记已解决」→ 状态已解决、显示"解决于 xxx，耗时 N 分钟"。

  

<img src="https://article-images.zsxq.com/FiX2fJcu-16acNlVcGxUHxtK2OYE" class="tiptap-image" alt="图片.png" />

  

<img src="https://article-images.zsxq.com/Fu2zVBoqqjbB-0JxV6fBSpKrZwSN" class="tiptap-image" alt="图片.png" />

  

<img src="https://article-images.zsxq.com/FlfZzvCVNf1qEHheYgRC0QD-1Hel" class="tiptap-image" alt="图片.png" />

**③ SLA 超时**：把某张在办工单的优先级改成"紧急"再点「检查超时」（或等 30 秒自动扫描）→ 列表该行标红、菜单角标显示"超时 N"、消息中心收到"SLA 预警"、工作台弹提醒。

  

<img src="https://article-images.zsxq.com/Ftg4_FYNaw7sSD4hM2Ku7SbYDw7j" class="tiptap-image" alt="图片.png" />

（想立刻看到效果：把「SLA 规则」里紧急的首次响应改成 1 分钟，新建一张紧急工单，等一分钟。）

\*\*④ 平台支持工单：\*\*企业侧「提交给平台支持」→ 填主题/优先级 → 提交；用平台账号登录 → 左侧「支持工单」→ 按租户号筛到它 → 认领 → 回复 → 标记已解决；回到企业侧：状态已跟着变，消息中心有「平台回复了支持工单 xxx」。

  

<img src="https://article-images.zsxq.com/FpiL_w29KDVjT550bIqHs2MwxC3b" class="tiptap-image" alt="图片.png" />

  

<img src="https://article-images.zsxq.com/Fm0MjDK86iudfXRYa-bp68zqnSpv" class="tiptap-image" alt="图片.png" />

\*\*⑤导出与权限：\*\*工单中心点「导出」→ 下载 CSV（Excel 打开中文不乱码）；把一个测试账号的角色改成 `QUALITY` 再登录 → 工单中心变只读（没有新建/认领/回复/SLA 规则按钮）。

  

<img src="https://article-images.zsxq.com/FvYUsGWIL5TfgfR-r8todSj88JF8" class="tiptap-image" alt="图片.png" />

## 总结

这一篇把"客户的问题"真正接住了：

1.  **工单不是记录表，是协作流程**：来源要能回到会话看上下文、处理人要能认领转派、状态要能按流程走、每一步都要留痕。少了任何一环，工单就退化成"另一个人工待办清单"。

2.  **SLA 要拆两段，而且必须快照**：一段管不住"回了不管"；不快照的话，改一次规则就能把存量工单的考核抹平。同理，升级不重算截止时间——**考核口径不能有后门**。

3.  **提醒要追着人跑**：长连接（当时在线）、消息中心（回头翻得到）、菜单角标（一眼看到），再加上"没人认领就发给全租户坐席"。只让列表变个颜色，等于没提醒。

4.  **权限要按角色收敛，还要能降级**：只读角色只看不动、SLA 规则只有管理员能改；但内部接口问不到角色时，日常作业照常放行（不能把客服堵死），只有管理类动作保守拒绝。

5.  **同一个表装两种工单，但鉴权要分开**：企业内部工单和平台支持工单共用 `ticket`，靠 `ticket_type` 区分；但接口前缀、鉴权口径、SLA 档位全都分开——混在一起最危险的不是功能，而是某天有人忘了校验租户，跨租户看见别人的工单。
